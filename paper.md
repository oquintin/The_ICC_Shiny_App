...
The ICC Shiny App was intentionally designed as an integrated platform rather than a collection of separate scripts or estimators. The choice of a Shiny web interface ensures accessibility to non-programmers while retaining the flexibility of R for advanced users. Multiple ICC estimators are implemented within a single interface to allow direct comparison under consistent inputs, making the sensitivity of ICC values to modelling assumptions explicit. The ICC database is implemented using REDCap rather than flat files to ensure structured data entry, secure storage, and long-term sustainability through institutional hosting. Educational material is embedded directly into the application to support both applied use and formal teaching.
...
The ICC Shiny App was designed to be used in teaching and methodological training on cluster-randomised trials and has been adopted by applied researchers during trial planning. The integration of a user-contributed ICC database hosted within institutional REDCap infrastructure provides a sustainable mechanism for accumulating and reusing empirical ICC evidence across studies. The need for such a tool is documented in multiple methodological reviews highlighting persistent problems in ICC reporting and use [@Ivers2011] [@Offorha2022]. By combining estimation, uncertainty handling, evidence synthesis, and education in a single platform, the app provides practical capabilities that were previously scattered across methodological literature and separate software tools.
...
No AI tools were used in the development of the software itself and the paper. Generative AI tools were used to assist with drafting parts of the documentation based on descriptions provided by authors. All content was reviewed, verified, and edited by the authors to ensure correctness and accuracy. 
